...
import { CollegePartnershipService } from './college-partnership.service';
import {
  CollegePartnershipEnquiry, CollegePartnershipStatus,
  COLLEGE_PARTNERSHIP_STATUS_LABELS, COLLEGE_TIMELINE_LABELS,
...
@Component({
  selector: 'snt-college-partnerships-admin',
  standalone: true,
  ...
  template: `
    <snt-page-shell
      ...
      subtitle="Manage college and university partnership enquiries"
      icon="🏫"
    >
      <ng-container slot="filters">
        <input class="filter-input" type="search" placeholder="Search by institution, city, contact…" [(ngModel)]="searchTerm" />
      ...
      @switch (state()) {
        ...
        @case ('ready') {
          @if (!filtered().length) {
            <snt-page-state type="empty" title="No enquiries found" description="No college partnership enquiries match your filters." />
          } @else {
                    ...
                    <tr>
                      <td>
                        <div class="applicant-cell">
                          <p class="font-medium">{{ e.collegeName }}</p>
                          <p class="text-muted">{{ e.programsInterested }}</p>
                        </div>
                      </td>
                      <td>
                        <div class="applicant-cell">
                      ...
                      <td class="text-sm">{{ e.city }}, {{ e.state }}</td>
                      <td class="text-sm">{{ e.numberOfStudents ?? '—' }}</td>
                      <td>
    ...
    <snt-drawer
      [open]="drawerOpen()"
      [title]="selected()?.collegeName ?? 'Enquiry Detail'"
      subtitle="College Partnership Enquiry"
      (closed)="drawerOpen.set(false)"
    >
      @if (selected(); as e) {
        <div class="detail-body">
          <div class="detail-section">
            <p class="detail-section-title">Institution Details</p>
            <div class="detail-grid">
              ...
              <div class="detail-field"><span class="detail-label">Students</span><span class="detail-value">{{ e.numberOfStudents ?? '—' }}</span></div>
              <div class="detail-field"><span class="detail-label">Mode</span><span class="detail-value">{{ e.mode }}</span></div>
              <div class="detail-field"><span class="detail-label">Timeline</span><span class="detail-value">{{ timelineLabel(e.timeline) }}</span></div>
              <div class="detail-field"><span class="detail-label">Programs</span><span class="detail-value">{{ e.programsInterested }}</span></div>
            </div>
            @if (e.message) {
...
export class CollegePartnershipsAdminComponent implements OnInit {
  private readonly svc        = inject(CollegePartnershipService);
  private readonly destroyRef = inject(DestroyRef);

  readonly state     = signal<LoadState>('loading');
  readonly enquiries = signal<CollegePartnershipEnquiry[]>([]);
  readonly drawerOpen = signal(false);
  readonly selected   = signal<CollegePartnershipEnquiry | null>(null);
  readonly saveState  = signal<'idle' | 'saving' | 'saved' | 'error'>('idle');

  searchTerm   = '';
  statusFilter = '';
  editStatus: CollegePartnershipStatus = 'new';
  editNotes = '';

  readonly statuses = Object.entries(COLLEGE_PARTNERSHIP_STATUS_LABELS).map(
    ([value, label]) => ({ value: value as CollegePartnershipStatus, label })
  );
  ...
  readonly filtered = computed(() => {
    ...
    return this.enquiries().filter((e) =>
      !term ||
      e.collegeName.toLowerCase().includes(term) ||
  ...
  openDetail(e: CollegePartnershipEnquiry): void {
    this.selected.set(e);
  ...
  statusLabel(s: CollegePartnershipStatus): string { return COLLEGE_PARTNERSHIP_STATUS_LABELS[s]; }
  timelineLabel(t: string): string { return COLLEGE_TIMELINE_LABELS[t as keyof typeof COLLEGE_TIMELINE_LABELS] ?? t; }

  statusVariant(s: CollegePartnershipStatus): 'success' | 'warning' | 'danger' | 'info' | 'neutral' {
    const map: Record<CollegePartnershipStatus, 'success' | 'warning' | 'danger' | 'info' | 'neutral'> = {
